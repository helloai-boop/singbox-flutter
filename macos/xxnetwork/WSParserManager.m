...
-(NSString *)save:(NSString *)json {
    ...
    NSString *path = [NSString stringWithFormat:@"%@/%@", self.workingDir.path, self.isGlobalMode ? @"global.json" : @"ai.json"];
    NSString *ai = [NSString stringWithContentsOfURL:[NSURL fileURLWithPath:path]];
    if (ai.length == 0) {
        return @"";
    }
...
-(NSString *)logDir {
    return [self.workingDir URLByAppendingPathComponent:@"box.log"].path;
}
