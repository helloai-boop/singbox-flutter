...
-(NSString *)getServerAddress {
    ...
    if ([protocol isEqualToString:@"vmess"] || [protocol isEqualToString:@"vless"]) {
        ...
        NSDictionary *node = vnext[0];
        NSString *address = node[@"address"];
        return address;
    }
    else if ([protocol isEqualToString:@"trojan"]) {
        NSDictionary *settings = self[@"settings"];
        NSArray *servers = settings[@"servers"];
        NSDictionary *node = servers[0];
        NSString *address = node[@"address"];
        return address;
    }
    return nil;
...
-(NSString *)getPassword {
    ...
    if ([protocol isEqualToString:@"vmess"] || [protocol isEqualToString:@"vless"]) {
        ...
        NSDictionary *node = vnext[0];
        NSArray *users = node[@"users"];
        NSDictionary *user = users[0];
        NSString *uuid = user[@"id"];
        return uuid;
    }
    return @"";
}

-(NSArray <NSString *> *)getTransportSettingDetails {
    NSString *protocol = self[@"protocol"];
    protocol = protocol.lowercaseString;
    
    NSDictionary *streamSettings = self[@"streamSettings"];
    if (!streamSettings) {
        return nil;
    }
    if (!streamSettings[@"network"]) {
        NSLog(@"Invalid network transport type");
        return nil;
    }
    NSString *transport = streamSettings[@"network"];
    transport = [transport lowercaseString];
    if ([transport isEqualToString:@"tcp"]) {
        
        NSDictionary *tcpSetting = streamSettings[@"tcpSettings"];
        if (!tcpSetting) {
            return nil;
        }
        NSMutableArray *list = [NSMutableArray new];
        NSDictionary *header = tcpSetting[@"header"];
        if (header[@"type"]) {
            [list addObject:header[@"type"]];
        }
        else {
            [list addObject:@"none"];
        }
        ...
        NSDictionary *request = header[@"request"];
        NSDictionary *headers = request[@"headers"];
        if (headers[@"Host"]) {
            [list addObject:headers[@"Host"]];
        }
        else {
            [list addObject:@""];
        }
        if (request[@"path"]) {
            [list addObject:request[@"path"]];
        }
        else {
            [list addObject:@""];
        }
    ...
    else if ([transport isEqualToString:@"kcp"]) {
        
        NSDictionary *kcpSetting = streamSettings[@"kcpSettings"];
        if (!kcpSetting) {
            return nil;
        }
        NSMutableArray *list = NSMutableArray.new;
        NSDictionary *header = kcpSetting[@"header"];
        if (header[@"type"]) {
            [list addObject:header[@"type"]];
        }
        else {
        ...
        [list addObject:@""];
        if (kcpSetting[@"seed"]) {
            [list addObject:kcpSetting[@"seed"]];
        }
        else {
            [list addObject:@""];
        }
    ...
    else if ([transport isEqualToString:@"ws"]) {
        
        NSDictionary *wsSetting = streamSettings[@"wsSettings"];
        if (!wsSetting) {
            return nil;
        }
        NSMutableArray *list = NSMutableArray.new;
        ...
        NSDictionary *header = wsSetting[@"headers"];
        if (header[@"Host"]) {
            [list addObject:header[@"Host"]];
        }
        else {
            [list addObject:@""];
        }
        if (wsSetting[@"path"]) {
            [list addObject:wsSetting[@"path"]];
        }
        else {
            [list addObject:@""];
        }
    ...
    else if ([transport isEqualToString:@"h2"]) {
        NSDictionary *h2Setting = streamSettings[@"httpSettings"];
        if (!h2Setting) {
            return nil;
        }
        NSMutableArray *list = NSMutableArray.new;
        ...
        if (h2Setting[@"host"]) {
            [list addObject:h2Setting[@"host"]];
        }
        else {
            [list addObject:@""];
        }
        if (h2Setting[@"path"]) {
            [list addObject:h2Setting[@"path"]];
        }
        else {
            [list addObject:@""];
        }
